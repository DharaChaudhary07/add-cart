let products =  [
    ...
    console.log(data);
    let row = document.getElementById('product');
    
    let CartStorage = JSON.parse(localStorage.getItem('cart')) || [];
    
    let count = document.getElementById('count');
    console.log(CartStorage.length);
    count.innerHTML = CartStorage.length;
    
    const displayCard = () => {
      row.innerHTML = '';
      products.forEach((product) => {
        // console.log(product);
        row.innerHTML += `<div class="card m-2" style="width: 18rem;">
        <img src="${product.thumbnail}" class="card-img-top" alt="...">
                   <div class="card-body">
                     <h5 class="card-title">${product.title}</h5>
                     <p class="card-text">${product.description}</p>
                     <p class="card-text">${product.price}</p>
                     <a href="#" class="btn btn-primary" onClick="return addCart(${product.id})">Add Cart</a>
                     </div>
                 </div>`
    ...
    const addCart =(id) =>{
      console.log(id);
      let singleRec = products.filter((ele) => {
        return ele.id == id;
      })
  
      let CartStorage = JSON.parse(localStorage.getItem('cart')) || [];
      
      let existingItem = CartStorage.find(cartItems => cartItems.id === singleRec[0].id)
  
      if(!existingItem){
        singleRec[0].quantity = 1;
        CartStorage = [...CartStorage, singleRec[0]];
      ...
      // }else{
      //   singleRec[0].quantity = 1;
      //   CartStorage.push(singleRec[0]);
      // }
      localStorage.setItem('cart', JSON.stringify(CartStorage));
      count.innerHTML = CartStorage.length;
  
      CartStorage = [...CartStorage, ...singleRec];
      localStorage.setItem('cart', JSON.stringify(CartStorage));
